...
util.oneOfGetter = function getOneOf(fieldNames) {
  ...
  return function () { // eslint-disable-line consistent-return
    for (let keys = Object.keys(this), i = keys.length - 1; i > -1; --i) if (fieldMap[keys[i]] === 1 && this[keys[i]] !== undefined && this[keys[i]] !== null) return keys[i]
  }
}
...
util.oneOfSetter = function setOneOf(fieldNames) {
  ...
  return function (name) {
    for (let i = 0; i < fieldNames.length; ++i) if (fieldNames[i] !== name) delete this[/* "_"+*/fieldNames[i]]// 设置为私有属性
  }
}

util.merge = function merge(dst, src, ifNotSet) { // used by converters
  for (let keys = Object.keys(src), i = 0; i < keys.length; ++i) if (dst[keys[i]] === undefined || !ifNotSet) dst[keys[i]] = src[keys[i]]
  return dst
}
