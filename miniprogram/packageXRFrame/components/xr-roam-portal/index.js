Component({
  ...
  methods: {
    ...
    handleTick() {
      ...
      const sceneMesh = this.scene.getElementById('scene-mesh-sea').getComponent(xrSystem.GLTF);
      const sceneMesh2 = this.scene.getElementById('scene-mesh-red').getComponent(xrSystem.GLTF);
      ...
      if (!this.inRealWorld) {
        ...
        sceneMesh2.meshes.forEach(mesh => mesh.material.setRenderState('stencilComp', 3));
      } else {
        // 虚拟世界
        ...
        sceneMesh2.meshes.forEach(mesh => mesh.material.setRenderState('stencilComp', 0));
      }
    },
    placeNode(event) {
