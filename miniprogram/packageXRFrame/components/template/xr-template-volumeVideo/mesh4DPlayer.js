...
class mesh4DPlayer {
  ...
  calcFrameIndex(data) {
    ...
    for (let a = 0; 16 > a; ++a) {
      // 遍历 16 个像素
      if (128 < this.pixelBuffer[4 * a] || 128 < this.pixelBuffer[4 * a + 1]) {
        // 如果像素值大于 128
  ...
  load() {
    let info_fn = this.urlRoot + '/mesh.json';
    console.log(info_fn)
  ...
  cacheVideo(url) {
    let storageKey = this.name + 'Video'
    let videoDownload = () => {
      wx.downloadFile({
        url: url,
        success: (res) => {
    ...
    let localVideoPath = wx.getStorageSync(storageKey)
    if (localVideoPath) {
      try {
  ...
  cacheBuffers() {
    let storageKey = this.name + 'Buffers'
    let localBuffersPathArr = wx.getStorageSync(storageKey)
    if (localBuffersPathArr) {
      try {
        this.buffers = localBuffersPathArr
        for (let i = 0; i < this.bufferIndexMaxL; i++) {
          let keybin = wx.getStorageSync(this.name + localBuffersPathArr[i].url)
  ...
  loadNextBuffer() {
    ...
    let cbFn = (res, urlkey) => {
      ...
      if (res) {
        ...
        try {
          let filePath = `${wx.env.USER_DATA_PATH}/${urlkey}`
          fs.writeFileSync(
            filePath,
            arrayBuffer,
          )
  ...
  async play() {
    ...
    await new Promise(resolve => {
      this.frameDataflag = setInterval(() => {
        if(this.decoder)
            frameData_0 = this.decoder.getFrameData()
        if (frameData_0) {
